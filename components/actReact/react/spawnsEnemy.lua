...
local spawnsEnemy = class("actReact.react.spawnsEnemy", Component)
...
function spawnsEnemy:action()
    if not self.maxenemycount or self.enemyCount < self.maxenemycount then
        local enemy = Actor:new(
            self.actor.world,
            self.actor.cache.x
                + self.spawnenemyoffsetx * self.actor.animationDirection
                + ((0.5 * self.actor.animationDirection + 0.5) * self.actor.width),
