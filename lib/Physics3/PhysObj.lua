...
function PhysObj:createTracers()
    ...
    table.insert(
        self.tracers.left,
        Physics3.Tracer:new(self, xOff, Physics3.TRACER_SIDE_TOP_DIST, Vector(-distance, 0))
    )
...
function PhysObj:checkCollisions()
    return self:leftColCheck() or self:rightColCheck() or self:bottomColCheck() or self:topColCheck()
end
...
function PhysObj:leftCollision()
end
function PhysObj:rightCollision()
end
